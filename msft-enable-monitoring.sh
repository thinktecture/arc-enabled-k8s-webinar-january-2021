...
validate_cluster_identity() {
  ...
  local identitytype=$(az resource show -g ${rgName} -n ${clusterName} --resource-type $resourceProvider --query identity.type -o tsv)
...
if [ -z $workspaceResourceId ]; then
  ...
  workspaceRegion=$(az resource show --ids ${workspaceResourceId} --query location -o tsv)
